...
const STATUS_META = {
  pending:     { label: 'Pending',     color: '#D97706', bg: '#FFFBEB', border: '#FCD34D', dot: 'o' },
  ...
  resolved:    { label: 'Resolved',    color: '#15803D', bg: '#F0FDF4', border: '#86EFAC', dot: 'v' },
  closed:      { label: 'Closed',      color: '#475569', bg: '#F8FAFC', border: '#CBD5E1', dot: 'x' },
};

const CAT_ICONS = { Electrical: 'E', Plumbing: 'P', Cleaning: 'C', Furniture: 'F' };
...
export default function FMDashboardScreen({ navigation }) {
  ...
  useEffect(() => { loadUser(); fetchIssues(); }, []);
  useEffect(() => { applyFilters(issues, search, statusF, categoryF); },
  ...
  return (
    <SafeAreaView style={styles.root}>
        ...
        <View style={styles.searchBox}>
          ...
          {search.length > 0 && (
            <TouchableOpacity onPress={() => setSearch('')}>
              <Text style={{ color: '#CBD5E1', fontSize: 16 }}>X</Text>
            </TouchableOpacity>
          )}
        </View>
      </View>
      ...
      <View style={styles.navBar}>
        ...
        <View style={styles.navItem}>
          <Text style={styles.navIcon}>List</Text>
          <Text style={styles.navLabel}>ISSUES</Text>
        </View>
        <View style={styles.navItem}>
          <Text style={styles.navIcon}>Work</Text>
          <Text style={styles.navLabel}>WORKERS</Text>
        </View>
        <View style={styles.navItem}>
          <Text style={styles.navIcon}>User</Text>
...
const styles = StyleSheet.create({
  ...
  statsRow:  { flexDirection: 'row', zIndex: 1 },
  statPill:  { flex: 1, marginRight: 8, backgroundColor: 'rgba(255,255,255,0.08)',
  ...
  searchIcon:  { fontSize: 13, marginRight: 6, color: '#64748B' },
  ...
  chip:        { marginRight: 8, paddingHorizontal: 14, paddingVertical: 7,
  ...
  catBox:      { width: 40, height: 40, borderRadius: 10, marginRight: 10,
                 justifyContent: 'center', alignItems: 'center' },
  catIcon:     { fontSize: 18, fontWeight: '800', color: '#0B1F3A' },
  cardTextBlock:{ flex: 1 },
  cardTitle:   { fontSize: 14, fontWeight: '800', color: '#0B1F3A' },
  ...
  tapHintRow:  { marginTop: 10, paddingTop: 10,
  ...
  emptyTitle: { fontSize: 20, fontWeight: '800', color: '#0B1F3A', marginTop: 4 },
  ...
  navIcon:   { fontSize: 11, marginBottom: 3, color: '#64748B' },
