...
export default function IssueDetailScreen({ route, navigation }) {
  ...
  const handleFinishWork = async () => {
    if (!photo) {
      Alert.alert('Completion Photo Required', 'Take or choose a completion photo before submitting.');
      return;
    }
    ...
    try {
      ...
      Alert.alert(
        '✅ Work Submitted',
        'The task was marked as resolved and the manager/member notifications were created.',
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.card}>
          ...
          <TouchableOpacity
            ...
            disabled={disabled}
          >
            {actionLoading ? <ActivityIndicator color="#fff" /> : <Text style={styles.finishBtnText}>✅ Submit Finished Work</Text>}
          </TouchableOpacity>

          {isResolved && <Text style={styles.doneNote}>This task is resolved and waiting for manager review.</Text>}
          {isClosed && <Text style={styles.doneNote}>This task has been closed by the manager.</Text>}
        </View>

        <View style={styles.photoSection}>
          <Text style={styles.photoTitle}>Uploaded Completion Photo</Text>
          {issue.completed_photo_url ? (
            <Image source={{ uri: issue.completed_photo_url }} style={styles.photo} resizeMode="cover" />
          ) : (
            <View style={styles.noPhoto}>
              <Text style={{ fontSize: 32 }}>🛠️</Text>
              <Text style={styles.noPhotoText}>No completion photo uploaded yet</Text>
            </View>
          )}
        </View>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  input: { minHeight: 95, borderWidth: 1, borderColor: C.border, borderRadius: 14, padding: 12, color: C.navy, backgroundColor: '#F8FAFC', textAlignVertical: 'top', fontWeight: '600' },
  secondaryAction: { borderWidth: 1, borderColor: C.teal, borderRadius: 14, paddingVertical: 13, alignItems: 'center', marginTop: 10 },
  secondaryActionText: { color: C.teal, fontWeight: '900', fontSize: 15 },
  actionBtn: { backgroundColor: C.teal, borderRadius: 14, paddingVertical: 14, alignItems: 'center', marginBottom: 12 },
  actionText: { color: '#fff', fontWeight: '900' },
  label: { color: C.navy, fontWeight: '900', marginBottom: 8 },
  uploadPlaceholder: { height: 145, borderRadius: 14, borderWidth: 1, borderColor: C.border, backgroundColor: '#F8FAFC', justifyContent: 'center', alignItems: 'center', marginBottom: 12 },
  previewPhoto: { height: 210, borderRadius: 14, backgroundColor: '#E2E8F0', marginBottom: 12 },
  photoBtn: { backgroundColor: C.navy, borderRadius: 14, paddingVertical: 14, alignItems: 'center', marginBottom: 10 },
  photoBtnText: { color: '#fff', fontWeight: '900' },
  photoBtnAlt: { backgroundColor: '#FFFBEB', borderWidth: 1, borderColor: '#FCD34D', borderRadius: 14, paddingVertical: 14, alignItems: 'center', marginBottom: 10 },
  photoBtnAltText: { color: '#92400E', fontWeight: '900' },
  finishBtn: { backgroundColor: C.green, borderRadius: 14, paddingVertical: 15, alignItems: 'center', marginTop: 4 },
  finishBtnText: { color: '#fff', fontWeight: '900' },
  doneNote: { color: C.green, fontWeight: '800', textAlign: 'center', marginTop: 12 },
  disabled: { opacity: 0.45 },
  primaryBtn: { backgroundColor: C.navy, borderRadius: 12, paddingVertical: 12, paddingHorizontal: 24, marginTop: 10 },
  primaryBtnText: { color: '#fff', fontWeight: '900' },
  secondaryBtn: { marginTop: 10, paddingVertical: 12, paddingHorizontal: 24 },
